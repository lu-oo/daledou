...
项目已经在 `cloudflare/.nvmrc` 和 `cloudflare/.node-version` 固定：
...
脚本会优先使用 `$HOME/.nvm/versions/node/v22.12.0/bin`，不会安装新的 Node 版本。
...
如果 `*.workers.dev` 子域访问出现 `1101`，但自定义域名可以正常访问，请以后都使用自定义域名作为 Worker 地址。例如：
...
- 配置中不包含 `cpu_ms`。
- Python 源任务数与 JS Worker 任务数一致：`noon=64`，`evening=71`。
- Cookie Secret 格式校验。
